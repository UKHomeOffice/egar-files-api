# API for Storing information on file for use in eGAR 
...
By default the service runs by default on port 8086. But to change this, add to the configuration the following:
```
--server.port=8080
```
...
```
## Running with SQS
To run against SQS we need to remove the TBC profile and add the following parameters:
```
...
The system will connect to a db instance on the same host with the database `dev_file_api`.
...
```
mvn -Dflyway.url=jdbc:postgresql://localhost/dev_file_api -Dflyway.user=postgres -Dflyway.password=mysecretpassword flyway:migrate 
```
